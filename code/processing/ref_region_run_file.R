...
#wrapper function for rr development and analysis
  ...
  for(i in 1:length(ref_region$US_L4CODE)){
    ...
    for(j in 1:nrow(all_treatments)){
     ...
     if(sf::st_geometry_type(aoi) == 'GEOMETRYCOLLECTION'){
      ...
      colnames(carbon_values) <- c('evc', 'carbon' ,'pixels')
      ...
      if(ks.test(aoi_vals$EVC, rr_pixels$evc)$p.value < 0.05){print(paste0('Violation! ', aoi$distid, ' project does not match RR'))}
      ...
      rr_evaluators$area <- 'ref_region'
      
      out <- dplyr::bind_rows(aoi_evaluators, rr_evaluators)
      #out <- full_stack_differences(project_tibble = aoi_evaluators, rr_tibble = rr_evaluators, vintage = 'new')
  ...
  df2 <-
    ...
    dplyr::group_by(Location,composite_2015_median,
                    ...
                    lat,lon,area, distid,ecoregion,tolerance,disttype,trt_yr) |>
    tidyr::pivot_longer(cols = -c('composite_2015_median','composite_2016_median','composite_2017_median',
                                  'composite_2018_median','composite_2019_median','composite_2020_median',
                                  'composite_2021_median','composite_2022_median','composite_2023_median',
                                  'Location','lat','lon','area', 'distid','ecoregion','tolerance','disttype','trt_yr'
    )) |>
    dplyr::reframe(
